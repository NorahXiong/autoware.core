...
void createSimpleProblem(Eigen::MatrixXd & P, Eigen::MatrixXd & A, std::vector<double> & q, std::vector<double> & l, std::vector<double> & u) {
    P.resize(2, 2);
    P << 1, 0,
         0, 1;
    A.resize(2, 2);
...
TEST(OSQPInterfaceTest, OptimizeMethodWithInvalidInput) {
    Eigen::MatrixXd P, A;
    std::vector<double> q, l, u;
    createSimpleProblem(P, A, q, l, u);
    P(0, 1) = 2; // Invalid input: nonpositive definite matrix
    OSQPInterface osqp(P, A, q, l, u);     
    EXPECT_NO_THROW(autoware::qp_interface::calCSCMatrixTrapezoidal(P)); 
    EXPECT_NO_THROW(autoware::qp_interface::calCSCMatrix(A));
    CSC_Matrix P_csc = autoware::qp_interface::calCSCMatrixTrapezoidal(P);
    CSC_Matrix A_csc = autoware::qp_interface::calCSCMatrix(A);
    std::cerr << __FILE__ << ": " << __LINE__ << std::endl;
    osqp.isSolved();
    std::cerr << __FILE__ << ": " << __LINE__ << std::endl;
    EXPECT_ANY_THROW(osqp.optimize(P_csc, A_csc, q, l, u)); 
}
...
TEST(OSQPInterfaceTest, GetMethodsUninitialized) {
    OSQPInterface osqp;
    EXPECT_ANY_THROW({
    ...
    // EXPECT_EQ(osqp.getStatus(), "OSQP_SOLVED");
    // EXPECT_FALSE(osqp.isSolved());
    // EXPECT_EQ(osqp.getPolishStatus(), 0);
    // std::vector<double> dual_solution = osqp.getDualSolution();
    // EXPECT_EQ(dual_solution.size(), 0);
}
